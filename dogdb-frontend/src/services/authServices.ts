...
import { SigninDataType, SignupFormDataType } from "@/lib/schemas";
...
export const signup = async (userData: SignupFormDataType) => {
  const response = await fetch(`${EXPRESS_URL}/api/v1/user/signup`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
